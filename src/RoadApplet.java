...
{  
	...
	private final int SLOW_INIT = 0;
	private final int DENS_INIT = 1;
	private final int SPEED_INIT = 1;
...
{  
	...
	void update(double slowdown, double density, int simspeed, int counter)
	{  
...
class Road
{
	...
	public static final int mp =LENGTH-1;
	// length of the measurement
	public static final int ml = LENGTH/3;
	// dummy for infinity
	public static final double DBL_INF = 99999.99;
	...
	void update(double prob_slowdown, double prob_create)
	{  
		// number of vehicles
	...
	void paint(Graphics g,int row,int dotdist,int dotsize,int xshift)
	{  
		for (int i = 0; i < LENGTH; i++)
		{
			if (speed[i] >= 0) 
	...
	void indypaint(Graphics g, int dotdist, int dotsize,int xm,int ym,int radius)
	{  
		// degrees per vehicle
	...
	void diagramAxis(Graphics g, int xOr, int yOr, int xDiag, int yDiag)
	{
	...
	void street(Graphics g, int xm, int ym, int r1, int r2)
	{
		g.setColor(streetcolor);
	...
	void ClearDiagrams(Graphics bg,int xOrigin[],int yOrigin[],int xDiagram[],int yDiagram[])
	{
	...
	void bluelines(Graphics g,int x1,int y1,int x2,int y2)
	{  
	...
	void SpaceTimeLabels(Graphics g,int x1,int y1,int xA1,int yA1,int length1,String label1,int x2,int y2,int xA2,int yA2,int length2,String[] label2)
	{  
	...
	static void drawArrow(Graphics g,int xA,int yA,int Length,int Width,int deg,boolean filled,Color color)
	{
